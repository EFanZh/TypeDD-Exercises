...
searchEntry term items = loop [] items where
    loop : List String -> Vect n String -> List String
    loop acc [] = reverse acc
    loop acc (x :: xs) = if isInfixOf term x then loop (x :: acc) xs else loop acc xs
